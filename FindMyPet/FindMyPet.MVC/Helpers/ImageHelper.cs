...
namespace FindMyPet.MVC.Helpers
{
    ...
    {
        void SaveJpeg(string path, Image img);
        void Save(string path, Image img, ImageCodecInfo imageCodecInfo);
        ImageCodecInfo GetEncoderInfo(string mimeType);
        Image PutOnCanvas(Image image, int width, int height, Color canvasColor);
        Image PutOnWhiteCanvas(Image image, int width, int height);
        Image Resize(Image image, int newWidth, int maxHeight, bool onlyResizeIfWider);
        Image Crop(Image img, Rectangle cropArea);
        byte[] imageToByteArray(System.Drawing.Image imageIn);
        Image byteArrayToImage(byte[] byteArrayIn);
        string GetImage(object img);
        void PerformImageResizeAndPutOnCanvas(string pFilePath, string pFileName, int pWidth, int pHeight, string pOutputFileName);
    }
    ...
    public class ImageHelper : IImageHelper
    {
        ...
        public void PerformImageResizeAndPutOnCanvas(string folderPath, string currentFileName, int widthSize, int heightSize, string newFileName)
        {
            var oldImageFilePath = Path.Combine(folderPath, currentFileName);
            var newImageFilePath = Path.Combine(folderPath, newFileName);

            System.Drawing.Image imgBef;
            imgBef = System.Drawing.Image.FromFile(oldImageFilePath);

            System.Drawing.Image _imgR;
            _imgR = this.Resize(imgBef, widthSize, heightSize, true);

            System.Drawing.Image _img2;
            _img2 = this.PutOnCanvas(_imgR, widthSize, heightSize, System.Drawing.Color.Black);

            //Save JPEG  
            this.SaveJpeg(newImageFilePath, _img2);
